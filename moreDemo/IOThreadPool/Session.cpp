...
void Session::start()
{
	...
	//m_socket.async_read_some(boost::asio::buffer(m_data, max_length), 
		//boost::asio::bind_executor(m_strand, std::bind(&Session::handle_read, this, std::placeholders::_1, std::placeholders::_2, shared_from_this())));

	//简单的粘包处理方式，读指定大小的头部，然后触发handle_read_head
	m_recv_head_node->clear();
	boost::asio::async_read(m_socket, boost::asio::buffer(m_recv_head_node->m_data, HEAD_TOTAL_LENGTH), 
		//回调函数绑定到strand上，由strand调用
		boost::asio::bind_executor(m_strand, std::bind(&Session::handle_read_head, this, std::placeholders::_1, std::placeholders::_2, shared_from_this())));
}
...
void Session::send(char* msg, int max_length, short msg_id)
{
	...
	boost::asio::async_write(m_socket, boost::asio::buffer(msg_node->m_data, msg_node->m_total_len), 
		//绑定到strand
		boost::asio::bind_executor(m_strand, std::bind(&Session::handle_write, this, std::placeholders::_1, std::placeholders::_2, shared_from_this())));
}

void Session::send(const std::string& content, short msg_id)
{
	//加锁保护
	std::lock_guard<std::mutex> guard(m_send_queue_lock);
	...
	//将消息放入发送节点
	m_send_queue.push(std::make_shared<SendNode>(content.c_str(), content.size(), msg_id));
...
void Session::handle_read_head(const boost::system::error_code& ec, std::size_t bytes_transferred, std::shared_ptr<Session> self_session)
{
	if (!ec)
	{
		...
		//指定读多少数据才触发回调函数
		boost::asio::async_read(m_socket, boost::asio::buffer(m_recv_msg_node->m_data, data_len), 
			//绑定到strand
			boost::asio::bind_executor(m_strand, std::bind(&Session::handle_read_msg, this, std::placeholders::_1, std::placeholders::_2, shared_from_this())));
	}
	else
	{
	...
	if (!ec)
	{
		...
		if (!m_send_queue.empty())
		{
			...
			boost::asio::async_write(m_socket, boost::asio::buffer(node->m_data, node->m_total_len), 
				boost::asio::bind_executor(m_strand, std::bind(&Session::handle_write, this, std::placeholders::_1, std::placeholders::_2, self_session)));
		}
	}
	else
	{
