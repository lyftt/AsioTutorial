...
void Session::start()
{
	...
	boost::asio::async_read(m_socket, boost::asio::buffer(m_recv_head_node->m_data, HEAD_LENGTH), std::bind(&Session::handle_read_head, this, std::placeholders::_1, std::placeholders::_2, shared_from_this()));
}
...
void Session::handle_read(const boost::system::error_code& ec, std::size_t bytes_transferred, std::shared_ptr<Session> self_session)
{
	if (!ec)
	{
		...
		while (bytes_transferred > 0)
		{
			...
			{
				//剩余未处理的数据不够头部长度
				if (bytes_transferred + m_recv_head_node->m_cur_len < HEAD_LENGTH)
				{
				...
				//剩余未处理的数据比头部长度大
				int head_remain = HEAD_LENGTH - m_recv_head_node->m_cur_len;   //需要读取的头部剩余部分
